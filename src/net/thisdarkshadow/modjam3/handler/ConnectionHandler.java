...
public class ConnectionHandler implements IConnectionHandler {
	...
	public void playerLoggedIn(Player player, NetHandler netHandler, INetworkManager manager) {
		...
		if (Config.event) {
			...
			netHandler.getPlayer().sendChatToPlayer(new ChatMessageComponent().createFromText(Format.DARK_YELLOW + message1 + message2 + message3));
			netHandler.getPlayer().sendChatToPlayer(new ChatMessageComponent().createFromText("          "));
			netHandler.getPlayer().sendChatToPlayer(new ChatMessageComponent().createFromText(Format.DARK_YELLOW + message4));
			netHandler.getPlayer().sendChatToPlayer(new ChatMessageComponent().createFromText(Format.DARK_YELLOW + "This message can be disabled in the configs."));
		}
	}
